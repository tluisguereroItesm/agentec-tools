...
Entrega el PDF de tres formas (par??metro `delivery`):
...
# Tab selectors
_TAB_CURP_ID = "#tab-01"          # "Clave ??nica de Registro de Poblaci??n" tab
...
def _wait_for_waf(page: Any, timeout_ms: int) -> None:
    ...
    while time.time() < deadline:
        ...
        if "Challenge" not in page.title() and "challenge" not in page.title().lower():
            return
    raise RuntimeError(
        "CURP_SITE_ERROR: El sitio gob.mx no carg?? a tiempo (WAF challenge). "
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "CURP_SITE_ERROR: No se encontr?? el bot??n de b??squeda. "
            ...
            try:
                ...
                if any(w in body_text for w in ["no existe", "no encontrado", "no se encontr??", "curp inv??lido", "no encontrada"]):
                    mode_info = "datos personales proporcionados" if search_mode == "datos" else f"CURP '{input_data.get('curp')}'"
                    raise RuntimeError(f"CURP_NOT_FOUND: No se encontr?? un registro para {mode_info}.")
                raise RuntimeError(
                    "CURP_SITE_ERROR: Timeout esperando el bot??n de descarga. "
                    "Posible CURP no encontrado o el sitio est?? lento."
                )
...
def main() -> None:
    ...
    if search_mode == "datos":
        ...
        if missing:
            print(json.dumps(build_error_result(
                f"MISSING_ARG: faltan campos para b??squeda por datos personales: {', '.join(missing)}",
                "MISSING_ARG",
            )))
            sys.exit(1)
    else:
        curp = (input_data.get("curp") or "").strip().upper()
        if not curp:
            print(json.dumps(build_error_result("MISSING_ARG: falta el par??metro 'curp'", "MISSING_ARG")))
            sys.exit(1)
    ...
    try:
        ...
        if delivery == "email":
            ...
            body_html = (
                input_data.get("body")
                or f"<p>Adjunto encontrar??s el comprobante de la CURP <strong>{curp_display}</strong>.</p>"
